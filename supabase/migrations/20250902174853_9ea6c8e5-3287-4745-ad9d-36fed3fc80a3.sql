...
CREATE OR REPLACE FUNCTION mask_contact_info(contact_data text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION get_masked_contact_info(org_id uuid)
RETURNS TABLE(
    id uuid,
    name text,
    description text,
...
CREATE OR REPLACE FUNCTION can_view_org_contacts(org_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
