...
-- Enhanced organization contact protection
-- Add rate limiting for organization contact access
CREATE OR REPLACE FUNCTION public.enhanced_org_contact_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log organization contact access with enhanced details
  INSERT INTO public.audit_log (
    user_id,
    action,
    table_name, 
...
-- Create trigger for enhanced organization contact logging
DROP TRIGGER IF EXISTS enhanced_org_contact_audit ON public.organizations;
CREATE TRIGGER enhanced_org_contact_audit
  AFTER SELECT ON public.organizations
  FOR EACH ROW
  EXECUTE FUNCTION public.enhanced_org_contact_access();
...
CREATE TRIGGER payment_access_audit
  AFTER INSERT OR UPDATE OR DELETE OR SELECT ON public.payments
  FOR EACH ROW
